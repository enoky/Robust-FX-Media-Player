...
def _clean_filename(name: str) -> str:
    ...
    # Remove "Track 01" or "01." prefixes
    name = re.sub(r"(?i)^track\s*\d+\s*[-.]?\s*", "", name)
    name = re.sub(r"^\s*\d{1,3}\s*[-._]\s*", "", name)
    # Handle "01. Artist" pattern specific
...
def _qualifier_penalty(value: str, expected: str, per_token: int) -> int:
    ...
    # Use lowercase for flexible matching but keep tokens intact
    value_lower = value.lower()
    expected_lower = expected.lower()
    
    value_tokens = set(re.split(r"\W+", value_lower))
    expected_tokens = set(re.split(r"\W+", expected_lower))
    
    # We only care if the RESULT has qualifiers that the EXPECTED does not.
    # e.g. Result: "Song (Live)", Expected: "Song" -> Penalty
    # e.g. Result: "Song", Expected: "Song (Live)" -> No penalty (usually)
    
    extra_tokens = (QUALIFIER_TOKENS & value_tokens) - expected_tokens
    return len(extra_tokens) * per_token
...
def _score_itunes_result(
    ...
    *,
    ...
    raw_track_name = str(item.get("trackName") or "").lower()
    ...
    if expected_title_norm and track_name:
        ...
        # Use raw string for qualifier penalty to detect "Live", "Remix" etc.
        score -= _qualifier_penalty(raw_track_name, expected_title_norm, 15) 
...
def _score_deezer_result(
    ...
    *,
    ...
    raw_track_title = str(item.get("title") or "").lower()
    ...
    if expected_title_norm and track_title:
        ...
        # Use raw string for qualifier penalty
        score -= _qualifier_penalty(raw_track_title, expected_title_norm, 15)
